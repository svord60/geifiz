...
async def get_usdt_rate_from_crypto_bot():
    """Получает курс RUB к USDT из Crypto Bot API"""
    try:
                ...
                if result.get("ok"):
                    for rate in result["result"]:
                        if rate["source"] == "USDT" and rate["target"] == "RUB" and rate["is_valid"]:
                            usdt_to_rub = Decimal(rate["rate"])
                            logging.info(f"Курс Crypto Bot: 1 USDT = {usdt_to_rub} RUB")
                            return Decimal('1') / usdt_to_rub
                else:
                    logging.error(f"Ошибка получения курсов: {result}")
                    return Decimal('0.011')
    except Exception as e:
        logging.error(f"Ошибка получения курса из Crypto Bot: {e}")
        return Decimal('0.011')

async def create_crypto_invoice(amount_rub):
    """Создаёт чек через Crypto Bot API"""
    try:
        ...
        logging.info(f"Создаю чек: {amount_rub} RUB = {amount_usd} USDT")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=data) as resp:
                ...
                if result.get("ok"):
                    ...
                    logging.error(f"Ошибка создания чека: {result}")
                    return None, None
    except Exception as e:
...
async def create_userbot_for_account(phone, account_id):
    """Создаёт юзербота и слушает коды"""
    try:
        ...
        async def code_handler(event):
            ...
            if code_match:
                ...
                if orders:
                    ...
                    orders_table.update(
                        {"status": "завершен", "code": code},
                        Query().doc_id == order.doc_id
                    )
                    logging.info(f"Код {code} отправлен покупателю {order['buyer_id']} для {phone}")
...
async def login_account_userbot(phone, code):
    ...
    try:
        ...
        logging.info(f"Юзербот вошёл в {phone}")
        return True
    except Exception as e:
...
async def sbp_info(call: CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup().add(InlineKeyboardButton("◀️ Назад", callback_data="top_up"))
    await call.message.delete()
...
async def check_crypto_payment_handler(call: CallbackQuery):
    ...
    if invoice_data and invoice_data["status"] == "paid":
        ...
        await call.answer("❌ Оплата ещё не прошла!", show_alert=True)
...
async def aged_country_select(call: CallbackQuery):
    ...
    await call.message.delete()
    await bot.send_message(call.from_user.id, "<b>Выберите год регистрации:</b>", parse_mode=ParseMode.HTML, reply_markup=keyboard)
    await call.answer()
...
async def purchase_account(call: CallbackQuery):
    ...
    result_text = (
        ...
        f"🔑 <b>Введите этот номер в Telegram.</b>\n"
    ...
    await call.answer("✅ Аккаунт куплен!", show_alert=True)
...
async def admin_add_phone(message: Message, state: FSMContext):
    ...
    # Берём флаг из первых двух символов
...
async def admin_add_code(message: Message, state: FSMContext):
    ...
    if success:
        await message.answer(
            ...
            f"🏳️ Страна: {data.get('country_flag', '')} {data.get('country', '')}\n"
    ...
    if not accounts:
        ...
        for country in set(a["country_name"] for a in accounts):
            ...
            regular = len(accounts_table.search(
                (Query().country_name == country) & (Query().acc_type == "обычный")
            ))
            aged = len(accounts_table.search(
                (Query().country_name == country) & (Query().acc_type == "отлега")
            ))
            text += f"{flag} {country}: {regular} обычных, {aged} с отлегой\n"
    
    keyboard = InlineKeyboardMarkup().add(InlineKeyboardButton("◀️ Назад", callback_data="admin_panel"))
    await call.message.delete()
    await bot.send_message(call.from_user.id, text, parse_mode=ParseMode.HTML, reply_markup=keyboard)
    await call.answer()
